<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-bold text-2xl text-gray-800 leading-tight">{{ __('Edit Announcement') }}</h2>
        </div>
    </x-slot>
    ...
    <div class="py-12 bg-[#fcfdfd] min-h-screen relative overflow-hidden">
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="bg-white rounded-3xl shadow-lg shadow-gray-200/50 border border-gray-100 p-8 relative overflow-hidden">
                ...
                <form action="{{ route('announcements.update', $announcement->announcement_id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea name="content" rows="8"
                            class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-primary-500 focus:ring-primary-500 transition-colors sm:text-sm" required>{{ old('content', $announcement->content) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="date" name="publish_date" value="{{ old('publish_date', $announcement->publish_date) }}"
                                class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-primary-500 focus:ring-primary-500 transition-colors sm:text-sm py-3" required>
                        ...
                        <div>
                            ...
                            <input type="date" name="expiry_date" value="{{ old('expiry_date', $announcement->expiry_date) }}"
