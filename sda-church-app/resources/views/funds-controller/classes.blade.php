        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-3 no-print">
                <x-export-dropdown :routes="[
                    'csv' => route('funds-controller.classes.export', array_merge(request()->query(), ['format' => 'csv'])),
                    'pdf' => route('funds-controller.classes.export', array_merge(request()->query(), ['format' => 'pdf']))
                ]" />
    ...
    <div class="py-12 bg-[#fcfdfd] min-h-screen print:min-h-0 relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8 relative z-10">
            ...
            <div class="hidden print:block text-center border-b-2 border-gray-800 pb-6 mb-8 mt-4">
                ...
                <h2 class="text-xl font-semibold text-gray-600 mt-1">Class Funds Report</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 no-print">
                ...
                <div class="group relative bg-white rounded-3xl border border-gray-100 p-8 shadow-sm hover:shadow-2xl hover:shadow-primary-900/10 hover:-translate-y-2 transition-all duration-500 overflow-hidden">
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-4xl font-black text-gray-900 tracking-tight">GHS {{ number_format($classFunds->sum('amount'), 2) }}</p>
                    </div>
                </div>
                ...
                <div class="group relative bg-white rounded-3xl border border-gray-100 p-8 shadow-sm hover:shadow-2xl hover:shadow-secondary-900/10 hover:-translate-y-2 transition-all duration-500 overflow-hidden">
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-4xl font-black text-gray-900 tracking-tight">{{ $classFunds->count() }}</p>
                    </div>
                </div>
                ...
                <div class="group relative bg-white rounded-3xl border border-gray-100 p-8 shadow-sm hover:shadow-2xl hover:shadow-gray-900/10 hover:-translate-y-2 transition-all duration-500 overflow-hidden">
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-2xl font-black text-gray-900 tracking-tight">
                            {{ $classFunds->first() ? $classFunds->first()->date_received->format('M d, Y') : '—' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div class="xl:col-span-1 no-print">
                    <div class="bg-white rounded-3xl shadow-lg shadow-gray-200/50 border border-gray-100 p-6 relative overflow-hidden">
                        ...
                        <form action="{{ route('funds-controller.classes.store') }}" method="POST" class="space-y-5">
                            @csrf
                            <div>
                                <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Select Class</label>
                                <select name="class_name" class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-primary-500 focus:ring-primary-500 transition-colors text-sm py-3" required>
                                    <option value="">— Choose Class —</option>
                                    <option value="Eld. Peter Seku class">Eld. Peter Seku class</option>
                                    <option value="Eld. Oklu class">Eld. Oklu class</option>
                                    <option value="Bro. Daniel Donkor class">Bro. Daniel Donkor class</option>
                                    <option value="Bro. Samuel Stiles class">Bro. Samuel Stiles class</option>
                                </select>
                                @error('class_name') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                            </div>

                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Amount (GHS)</label>
                                    <input type="number" step="0.01" name="amount" class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-primary-500 focus:ring-primary-500 transition-colors sm:text-sm py-3" placeholder="0.00" required>
                                    @error('amount') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Date</label>
                                    <input type="date" name="date_received" value="{{ date('Y-m-d') }}" class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-primary-500 focus:ring-primary-500 transition-colors sm:text-sm py-3" required>
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden print:border-none print:shadow-none">
                        ...
                        <div class="p-6 border-b border-gray-50 bg-gray-50/30 no-print">
                            <div class="flex flex-wrap gap-3 items-center justify-between">
                                ...
                                <form method="GET" action="{{ route('funds-controller.classes') }}" class="flex flex-wrap gap-2 items-center">
                                    <input type="text" name="search" placeholder="Search receipt or class…"
                                    ...
                                    @if(request('search') || request('start_date') || request('end_date'))
                                        <a href="{{ route('funds-controller.classes') }}" class="px-4 py-2 bg-gray-100 text-gray-700 text-xs font-bold uppercase tracking-wider rounded-xl hover:bg-gray-200 transition-all">
                                            Clear
                                        </a>
                                    @endif
                                </form>
                            ...
                            <table class="min-w-full">
                                <thead class="bg-gray-50/50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Class</th>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="px-6 py-16 text-center">
                                                ...
                                                <p class="text-sm text-gray-400 font-medium">No class funds recorded yet.</p>
                                                <p class="text-xs text-gray-300 mt-1">Use the form to record the first entry.</p>
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                                @if($classFunds->count() > 0)
                                <tfoot class="bg-gray-50/80 border-t-2 border-gray-100">
                                    <tr>
                                        <td colspan="2" class="px-6 py-3 text-xs font-black text-gray-500 uppercase tracking-widest">
                                            {{ $classFunds->count() }} {{ Str::plural('record', $classFunds->count()) }}
                                        </td>
                                        <td class="px-6 py-3 text-right">
                                            <span class="text-sm font-black text-gray-900">GHS {{ number_format($classFunds->sum('amount'), 2) }}</span>
                                        </td>
